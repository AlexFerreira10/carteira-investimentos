...
st.write("Nesta seção, utilizamos técnicas de IA para prever preços de fechamento de ações.")
...
# Verificar se ainda há dados após remover nulos
# Estava com problema por conta da margem de dados insuficiente, pois também não é usados valores nulos.
if df.empty:
    st.error(f"Após a remoção de valores nulos, não há dados suficientes para análise para o ticker {ticker}.")
else:
    # Criar as features, com as outras estavam dando muito erro
    X = df[['Close', 'Volume']].values  # Agora temos o preço de fechamento e o volume como features
